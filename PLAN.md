...
3. **Weapons — exact dupe pass:** group only by item Hash plus the measured complete exact-roll fingerprint (never name — reissues collide). The fingerprint uses the named contiguous pre-tracker `Perks N` prefix, strips only DIM's selected `*` marker, retains complete perk names including any leading `Enhanced ` text, and excludes tracker/current socket, mod, masterwork, memento, and later mutable state. Different fingerprints and uncertain/truncated identities survive independently and never auto-junk, including Exotic rows whose Hashes are shared by distinct prefixes. Inside an exact group, rank copies (wishlist match > gear tier > masterwork tier > crafted level > stat total), then use a deterministic opaque-id tie-break independent of CSV order; best copy survives, rest → junk. Ties are noted as `dupe-tie` rather than `dupe-lower`; soft-protected copies get `#vc-review` notes instead of tags.
