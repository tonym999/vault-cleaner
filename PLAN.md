...
3. **Weapons — exact dupe pass:** group only by item Hash plus the measured complete exact-roll fingerprint (never name — reissues collide). The fingerprint uses the named contiguous pre-tracker `Perks N` prefix at the export's available width (`Perks 0` is the minimal schema invariant), strips only DIM's selected `*` marker, retains complete perk names including names beginning with the literal `Enhanced` followed by one separator space, and excludes tracker/current socket, mod, masterwork, memento, and later mutable state. The measured structural boundary is exactly `Kill Tracker` or `Crucible Tracker`, compared case-insensitively after removing one trailing selected marker. Unknown or future names merely ending in `Tracker` remain identity cells before a later measured boundary and make the row ungroupable when no measured boundary exists. Perk cells stay whole, including legitimate commas; adjacent cells are the measured multi-option representation. Comma-bearing cells containing either measured tracker label are ungroupable regardless of component order or marker placement. Different fingerprints and uncertain/truncated identities survive independently and never auto-junk, including Exotic rows whose Hashes are shared by distinct prefixes. Inside an exact group, wishlist matching no longer ranks copies because exact members share the roll identity; rank by gear tier > masterwork tier > crafted level > stat total, then use a deterministic opaque-id tie-break independent of CSV order. Best copy survives, rest → junk. Ties are noted as `dupe-tie` rather than `dupe-lower`; soft-protected copies get `#vc-review` notes instead of tags.
4. **Weapons — coverage pass (review-only):** group by item Hash; among copies left undecided by earlier passes and holding distinct, known exact-roll identities, flag copies whose useful curated keep-roll combinations are wholly contained in another copy's (`coverage-dominated by`) or which provide no curated combinations while another copy does (`coverage-uncovered vs`). Absorbed combinations are read from wishlists rather than enumerated from the export; comparison uses uncollapsed matched sets while combination counts collapse subsumed recommendations. Pairwise, deterministic (tie-broken by lowest opaque id), review-only: never tags junk and never changes an existing tag.
...
6. **Armor — close-dupe pass (M6, review-only):** flag dominated and near-identical pieces among the survivors for manual review; never tags junk.
7. **Armor — score pass:** score each legendary piece against configurable stat archetypes (v1 ships with Melee-primary and a generic spike profile). Keep top-N per slot per class; set-bonus armor gets a configurable score bonus so mediocre-stat pieces from active sets survive. Below floor → junk — except the vault's last kept copy of a (set-piece, archetype) combination, which is demoted to review instead (#30): scoring alone must never foreclose a set/build option.
8. **Everything unmatched:** left untagged — the tool only tags what it has a reason for.
...
3. **M3 — Wishlists:** choosy_voltron + Aegis download/parse/match, integrated with wishlist-trash protection.
...
exposes live, complete DIM `id:` search text for the weapon proposals matching
its current filters. Existing filters—including Session verdict—define the set;
there is no separate checkbox-selection state. Active saved vetoes do not hide
otherwise matching proposals, so the locating-only warning names their possible
inclusion. The query reuses #117's opaque-id validation and bounded chunking,
refreshes through the shared shown-summary render path, and changes no review,
item, DIM, or server state.
