...
- The fingerprint will use `Hash` plus every pre-tracker perk cell by header
  ...
  and no order changes were observed. Exotic rows also carry pre-tracker
...
- Implemented `exact_roll_fingerprint()` in `rules/dupes.py`: named `Perks N`
  ...
  order is preserved; exotic prefixes are included because Hash-only identity
  was not supported by the measurement.
