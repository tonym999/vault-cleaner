...
- The fingerprint will use `Hash` plus every pre-tracker perk cell by header
  name, stripping DIM's trailing selected `*` marker and a leading
  `Enhanced ` decoration. Perk options are retained in their measured socket
  order: the export represents multi-option sockets as adjacent named cells,
  and no order changes were observed. Exotic rows have no randomized roll
  prefix in this export, so their fixed `Hash` is the complete roll identity.
...
- Aggregate baseline: 117 same-Hash groups of at least two rows (346 rows,
  229 redundant rows). The measured candidate has 3 exact groups (14 exotic
  rows, 11 redundant rows); no legendary exact group was present. The
  candidate therefore bounds automatic cleanup to 11 rows before rails, rather
...
- Implemented `exact_roll_fingerprint()` in `rules/dupes.py`: named `Perks N`
  ...
  selected `*` and leading `Enhanced ` display decoration removed. Socket
  order is preserved; fixed-roll exotics use their non-empty `Hash` alone.
...
- Added a fake, synthetic Slammer-like export and behavioral coverage for
  ...
  multi-option cells, base/enhanced names, mutable state, unknown identity,
...
- Post-change private-export aggregate dry runs: 665 rows remained
  groupable under the measured contract; 3 exact groups covered 14 rows (11
  pre-rail redundant copies), with group sizes 9, 3, and 2, and the resolver
  emitted 8 review decisions with no automatic junk decisions. The synthetic
  Slammer dry run has one four-copy Hash with three distinct fingerprints and
  one two-copy exact group; it emitted one junk decision from that exact pair
  and kept distinct rolls separate.
- Focused validation passed: Ruff and 85 exact-dupe/weapon/rail/report tests.
  The full suite result was 719 passed, 1 skipped, 4 failed, and 2 errors;
