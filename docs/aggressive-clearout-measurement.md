...
- **`Id`:** Opaque string end-to-end. Handled as a string in `parse.py`, stored as string in `Decision.id`, formatted with surrounding triple-quotes on export ([report.py:100](../src/vault_cleaner/report.py#L100)), and ordered by `id_order.instance_id_order` without numeric conversion — note the order is **not** lexical for ordinary ids: an all-ASCII-digit id sorts by magnitude, keyed `(0, len(magnitude), magnitude, raw)` after stripping leading zeros, with the raw string only as the final tie-break; lexical ordering applies solely to non-decimal ids, keyed `(1, raw)` ([id_order.py:14-25](../src/vault_cleaner/rules/id_order.py#L14-L25)). Children preserving the deterministic tie-break must preserve magnitude order, not string order.
...
|---|---|---|---|---|---|---|---|
| **Choosy Voltron** (configured, `choosy_voltron`) | Multi-curator community — curator names measured present in the cached file (see below) | Broad PvE & PvP baseline; **completeness unknown and NOT MEASURED** — the captured counts measure 1,234 item hashes in the cached file, but they were never compared against the current Bungie manifest or against the real export's 440 hashes, so no coverage claim over "all Destiny history" is supported. #140 likewise records exhaustive refreshed-weapon coverage as unestablished. See §14 item 9. | 255,373 keep rolls across 1,234 items; 53 trash entries across 53 items ([evidence §7](evidence/issue-142/README.md#7-wishlists-command-output-and-local-cache-state), re-verified 2026-09-06) | None (binary keep/trash) | Mixed loss: ~1% of `dimwishlist:` lines carry an inline `#notes:` tail discarded at `LINE_RE` (`wishlist.py:29`); the remainder of its notes are standalone `//notes:` lines discarded at `wishlist.py:73-74`, same as the two Aegis sources — see the measured seam table above | [`2026-08-03T22:53:37Z`](https://github.com/48klocs/dim-wish-list-sources) upstream (retrieved 2026-09-06); local cache `2026-09-06` (evidence §7) | Hash-based; enhanced perk hashes listed explicitly |
...
Ciceron, MrCharles, and Nitaraku are **not independent curators**. All three are mechanical scripts converting a single source of truth: Aegis's PvE Endgame spreadsheet ([https://docs.google.com/spreadsheets/d/1JM-0SlxVDAi-C6rGVlLxa-J1WGewEeL8Qvq4htWZHhY](https://docs.google.com/spreadsheets/d/1JM-0SlxVDAi-C6rGVlLxa-J1WGewEeL8Qvq4htWZHhY); retrieved/confirmed 2026-09-06, cited from the `// Tierlist source:` comment header in the cached `wishlists/aegis.txt` and independently from the Ciceron repository README's "Endgame Analysis Spreadsheet" link — the `wishlists/aegis.txt` header was read from the **existing fresh cache**, not re-fetched (§5 and evidence §7 record that run as cache-served), while the Ciceron README **was** fetched this session via `gh api`). Counting agreement between them as "consensus" is a statistical error. They must be treated as **one curation family** (Aegis PvE Endgame), evaluated against Choosy Voltron as the broad community baseline.
...
1. Server: [src/vault_cleaner/server/app.py:795-799](../src/vault_cleaner/server/app.py#L795-L799)
2. CLI: [src/vault_cleaner/cli.py:488-492](../src/vault_cleaner/cli.py#L488-L492)
...
- **Unequipped character weapons ($C$):** 100 ($109 - 9$) — derived by subtracting all equipped rows from the character rows, which is sound only if no row is both `Owner == "Vault"` and `Equipped == "true"`. The fence never tested that, and the export is unavailable to re-test it, so this figure and the $D_v + D_c \ge 190$ projection below assume it. Recorded in §14 item 10; a corrected derivation counting $C$ directly is specified — and deliberately left unexecuted — in [evidence §8, "Corrected derivation procedure"](evidence/issue-142/README.md#corrected-derivation-procedure-round-7--specified-not-executed).
...
- **Required removals for 100-space goal:**
  Given estimated $F \approx 10$ and $C = 100$ — both inputs carry uncertainty the figure inherits: $F$ is an owner estimate, not an export field (§10), and $C$ assumes no vault-owned row is equipped (§14 item 10). A higher true $C$ raises this figure one-for-one.
  ```text
...
10. **Absence of `Owner == "Vault"` rows carrying `Equipped == "true"` on the real export (round 7):** evidence §8's original fence derived the unequipped-character count as `total_char - eq_count`, subtracting every equipped row from the character rows. That is sound only if no row is simultaneously owned by the vault and equipped. The DIM schema does not enforce that invariant, and the fence never tested it, so the recorded `C = 100` — and the `Dv + Dc >= 190` projection in §11 and §13 that derives from it — assumes rather than establishes it. A corrected derivation that counts `C` directly and reports any such inconsistent rows is specified in evidence §8's "Corrected derivation procedure" section, deliberately carrying no claimed output: the captured fence and its output are left as the record of what produced these figures, and the corrected form has not been executed because the export is on another machine and unavailable to this session (same limitation as items 1, 3, 5, 6 and 9 above). If any vault-equipped row exists on that export, the true `C` is higher than 100 and the required-removal figure rises with it. **NOT MEASURED.**
