...
- **Unequipped character weapons ($C$):** 100 ($109 - 9$) — derived by subtracting all equipped rows from the character rows, which is sound only if no row is both `Owner == "Vault"` and `Equipped == "true"`. The fence never tested that, and the export is unavailable to re-test it, so this figure and the $D_v + D_c \ge 190$ projection below assume it. Recorded in §14 item 10; the corrected derivation now counts $C$ directly.
