...
7. **Handle Review Findings & Legacy Plans:**
   ...
   - **Review-fix diff audit (every model):** when a fix round returns,
     compare `git diff <previous_reviewed_head>...<new_head>` with the
     implementer's claimed summary and account for every hunk. For changed
     documentation, inspect prose with
     ...
     never a substitute for this audit.
...
For any review-fix round, treat the implementer's completion summary as
...
when the requested fix is correct and every automated gate passes (PR #160).
